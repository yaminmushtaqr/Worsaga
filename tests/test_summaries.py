...
class TestScoreLine:
    ...
    def test_blog_admin_penalised(self):
        ...
        content = "Portfolio theory explains how diversification lowers risk across assets"
    ...
    def test_long_comma_list_penalised(self):
        junk = "CAD, development, gamification, virtualization, accounting, collaboration, client relationship management, Management"
...
class TestBuildDeterministicSummary:
    ...
    def test_filters_long_comma_lists(self):
        ...
        text = (
            ...
            "collaboration, client relationship management, Management\n\n"
            "Firms maximize profit where marginal cost equals marginal revenue\n\n"
            "Comparative advantage determines trade patterns between nations"
        )
        file_texts = [("slides.pptx", text)]
    ...
    def test_filters_parenthetical_example_and_quote_fragments(self):
        ...
        text = (
            "Total firm value equals equity value plus debt minus cash\n\n"
            "(ignoring fixed assets, which are very small)\n\n"
            "Example: exit in 2014 at 6x multiple implies a firm value of $210 million\n\n"
            "We’d have to be careful ---we don’t want to hire a bunch of unproductive people. But I think we could pull this off.”"
        )
        file_texts = [("slides.pdf", text)]
        bullets = build_deterministic_summary(file_texts)
        ...
        assert "total firm value equals equity value plus debt minus cash" in combined
...
class TestBuildWeeklySummary:
    ...
    def test_download_error_becomes_warning_not_crash(self):
        ...
        class _FailingClient(_StubClient):
            def download_file(self, fileurl, *, max_bytes=None):
                raise DownloadError("oversize", "'w1.pdf' exceeds the limit; skipped.")

        client = _FailingClient(sections=sections)
        result = build_weekly_summary(client, 42, 1)
        assert result["method"] == "fallback"
        assert len(result["warnings"]) == 1
        assert "w1.pdf" in result["warnings"][0]
        assert "Skipped" in result["warnings"][0]

    def test_successful_summary_has_empty_warnings(self):
        sections = [
            _make_section("Week 1: Intro", 1, [
                _make_module(10, "Slides", contents=[
                    _make_file_content("w1.txt"),
    ...
    def test_summary_caps_downloaded_files(self):
        from worsaga.summaries import MAX_SUMMARY_FILES

        contents = [
            _make_file_content(f"w1-part{i}.txt") for i in range(10)
        ]
        sections = [
            _make_section("Week 1: Intro", 1, [
                _make_module(10, "Slides", contents=contents),
            ]),
        ]
        client = _StubClient(sections=sections, file_bytes=b"some text content")
        build_weekly_summary(client, 42, 1)
        assert len(client.downloaded) <= MAX_SUMMARY_FILES
