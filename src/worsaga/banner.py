...
def _rich_banner_large(*, version: str) -> None:
    ...
    body.append("  —  Moodle-first, local-first, read-only", style="dim rgb(120,45,45)")
    body.append("\n")
...
def _rich_banner_compact(*, version: str) -> None:
    ...
    detail = Text("  Moodle-first, local-first, read-only", style="dim rgb(120,50,50)")
...
def _ansi_banner(*, version: str) -> None:
    ...
    lines = [
        ...
        f"{_DR}  │{_RST}  {_DIM}Moodle-first, local-first, read-only  {_RST}{_DR}│{_RST}",
        f"{_DR}  └{'─' * 38}┘{_RST}",
    ]
    sys.stdout.write("\n".join(lines) + "\n")
