...
def strip_file_urls(materials: list[dict]) -> list[dict]:
    """Return copies of material records without raw ``file_url`` values.

    Raw Moodle file URLs require token authentication and are kept out
    of default user-facing JSON and MCP responses. Downloads always go
    through the authenticated download path instead.
    """
    return [
        {key: value for key, value in material.items() if key != "file_url"}
...
def download_material(
    ...
    *,
    ...
    ------
    DownloadError
        Categorized failure (auth, not_found, network, oversize,
        invalid_url, empty) with a token-free message. Oversize files
        are skipped entirely — no partial file is ever written.
    """
