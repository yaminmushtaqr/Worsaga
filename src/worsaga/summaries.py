...
MAX_BULLETS = 6
MIN_BULLET_SCORE = 18
MAX_SUMMARY_FILES = 6  # hard cap on files downloaded per summary
...
def build_deterministic_summary(
    ...
    *,
    ...
    for _, raw_text in file_texts:
        # Aggressive tier: captions, source lines, and reading-list
        # entries make poor bullets even though extraction keeps them.
...
def build_weekly_summary(
    ...
    *,
    ...
    if section and section.get("modules"):
        files = get_downloadable_files(section["modules"])[:MAX_SUMMARY_FILES]
        for finfo in files:
            url = finfo["fileurl"]
            if not url:
                continue
            ...
            try:
                ...
                warnings.append(f"Skipped {finfo['filename']}: {exc}")
                continue
            if data:
